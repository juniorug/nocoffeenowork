<?
    ...
    echo '$count_produtos_ativos: '.$count_produtos_ativos."<br>";
    if ($person_id) {
	  for ($product_id = 1; $product_id <= $count_produtos_ativos; $product_id++) {
		...
		echo '$qtde_produto: '.$qtde_produto."<br>";
		if ($qtde_produto) {
			$call_proc =  'CALL insert_PPP('.$person_id.','.$product_id.','.$purchase_id.','.$qtde_produto.')';				
			if (!$conn->query($call_proc)) {
			    echo "CALL failed: (" . $mysqli->errno . ") " . $mysqli->error;
				$msg = 'Houve um erro ao tentar alterar o pedido, por favor entre em contato com o administrador. ERR02';
				$ok = 2;
			} else {
			    echo 'FOIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII';
			...
			if (!$conn->query($call_proc)) {
			    echo "CALL failed: (" . $mysqli->errno . ") " . $mysqli->error;
				$msg = 'Houve um erro ao tentar alterar o pedido, por favor entre em contato com o administrador. ERR03';
				$ok = 3;
			} else {
				$msg = 'Obrigado! Seu pedido foi adicionado ao inventário. Nós entraremos em contato com você em breve.';
				$ok = 1; 

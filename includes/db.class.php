<?
  ...
  class DB
  {
    ...
    function DB($base, $server, $user, $pass)
    {
	  ...
	  $this->mysqli = $link;
      //mysql_connect($server, $user, $pass) or die('Não foi possível conercta ao servidor.');
      //mysql_select_db($base)               or die('Não foi possível selecionar o banco de dados.');
    }
  ...
  function consulta($sql){
    ...
    $this->retorno = mysqli_query($this->mysqli, $sql) or die(mysqli_error());
  
    return $this->retorno;
  }
  
  
  # Retorna o numero de linhas recebidos de uma consulta
  function nun_linhas($x){
  
    return mysql_num_rows($x);
  }
  
  # Obtem uma linha como uma matriz associativa, uma matriz numerica, ou ambas 
  function lista($x){
  
    return mysql_fetch_array($x); 
  }
  
  /*  Envia uma consulta e faz uma matriz do primeiro resultado obtido */
  function dados($sql){
  
    //return mysql_fetch_assoc(mysqli_query($this->mysqli, $sql));
    return mysql_fetch_array(mysqli_query($this->mysqli, $sql));
  }
  
  
  /* Insere dados no banco de dados de forma facilitada // essa funcao precisa ser reparada */
  
  function insereArray( $info, $dados, $acao, $id){
    ...
    if($acao == 'atualiza'){
      ...
      $sql = 'UPDATE '.$info['tabela'].' SET '.$str.' WHERE '.$info['id_tabela'].'='.$id .' LIMIT 1';
      ...
      $sql = 'INSERT INTO '.$info['tabela'].' ('.$campos.') VALUES('.$valores.')';
    }
    
    $insere = mysqli_query($this->mysqli, $sql) or die(mysqli_error());
  ...
  function insert($table, $data)
   ...
   /* function query($query, $debug = -1)
    ...
    {
      $this->nbQueries++;
      mysqli_query($this->mysqli,$query) or $this->debugAndDie($query);
